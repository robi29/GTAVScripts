...
__forceinline void update()
{
	...
	if (!switchOn || !ENTITY::DOES_ENTITY_EXIST(playerPed) || !PLAYER::IS_PLAYER_CONTROL_ON(player) || PLAYER::IS_PLAYER_DEAD(player) || PLAYER::IS_PLAYER_BEING_ARRESTED(player, TRUE))
	{
		//sprintf_s(text, "%s2\n", text);
...
void load()
{
	...
	float value;
	FILE* f = NULL;
	f = fopen("advancedblur.ini", "r");
	if (f != NULL)
	{
		while (fscanf(f, "%s %f", name, &value) != EOF)
		{
			if (!strcmp(name, "PedBlur"))
				pedBlur = value;
			else if (!strcmp(name, "VehicleBlur"))
				vehBlur = value;
			else if (!strcmp(name, "VehicleStartBlur"))
				vehStart = value;
		}
		fclose(f);
	}*/
...
void ScriptMain()
{
	/*std::fstream file;
	std::string shit;
	file.open("advancedblur.ini", std::ios_base::in);
	if (file.good())
	{
		file >> shit;
		file >> pedBlur;
	...
	while (true)
	{
		if (IsKeyJustUp(VK_DELETE))
		{
			...
			//sprintf_s(text, "jo %d", (int)switchOn);
			//set_status_text(text);
			if (switchOn)
			{
				load();
