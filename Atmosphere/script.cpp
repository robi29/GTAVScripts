...
__forceinline void update( Entity entity )
{
    ...
    else if( coords.z < spaceZoneStart[static_cast<int>( currentWeatherType )] ) // z < 3000
    {
        ...
        GAMEPLAY::_SET_WEATHER_TYPE_TRANSITION( weatherHashes[aboveWeatherIndex], weatherHashes[spaceWeatherIndex], SYSTEM::SIN( ( coords.z - aboveCloudsZoneEnd[static_cast<int>( currentWeatherType )] ) / ( spaceZoneStart[static_cast<int>( currentWeatherType )] - aboveCloudsZoneEnd[static_cast<int>( currentWeatherType )] ) * 90 ) );
